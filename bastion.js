/* The Bastion Bastion Manager (vanilla JS)
   Data: /data/facilities.json, /data/tools_tables.json, /data/events.json
   State: localStorage 'bastion_bastion_state_v1'
*/
...
(() => {
  ...
  const USE_TEST_STORAGE = false;

  const STORAGE_KEY_PROD = "scarlettWorldHub.bastion.v1";
  const STORAGE_KEY_TEST = "scarlettWorldHub.bastion.v1_TEST";
  const STORAGE_KEY = USE_TEST_STORAGE ? STORAGE_KEY_TEST : STORAGE_KEY_PROD;

  const state = loadState();

  function loadHubMeta(){
    try { return JSON.parse(localStorage.getItem("scarlettWorldHub.v1") || "{}"); } catch { return {}; }
  }

  function applyWorldHubBranding(){
    const hub = loadHubMeta();
    const bastionName = String(hub?.campaign?.bastionName || "Your Bastion").trim() || "Your Bastion";
    const titleEl = document.querySelector(".brand__title");
    const subEl = document.querySelector(".brand__sub");
    const pageTitle = `${bastionName} • Bastion Manager`;
    if (titleEl) titleEl.textContent = pageTitle.toUpperCase();
    if (subEl) subEl.textContent = "Scarlett Isles World Hub • Generic Bastion Turn Tools";
    document.title = pageTitle;
  }

  applyWorldHubBranding();
...
ui.clearArtisanToolsBtn?.addEventListener("click", ()=>{
  ...
  render();
    ...
    ui.btnFormClan?.addEventListener("click", async () => {
      ...
      const res = await openSIModalChoice({
        title: "Form Clan",
        bodyHtml: `
          <div class="field"><span>Clan Name</span><input id="siClanName" type="text" placeholder="e.g. Clan Bastion" /></div>
    ...
    ui.btnFormMerc?.addEventListener("click", async () => {
      ...
      const res = await openSIModalChoice({
        ...
        bodyHtml: `
          <div class="field"><span>Brigade Name</span><input id="siMercName" type="text" placeholder="e.g. The Bastion Freeblades" /></div>
          <div class="small muted" style="margin-top:10px">This is persistent. Save is in TEST mode right now.</div>
        `,
  ...
  state.turn += 1;
       ...
       // Auto-resolve Bastion Trade Network routes once per Bastion Turn
...
ui.downloadSaveBtn?.addEventListener("click", () => {
  try{
    ...
    a.download = `bastion_bastion_save_turn_${state.turn}.json`;
...
ui.clearBastionMapBtn?.addEventListener("click", () => {
  ...
  // Custom completion branch: Bastion Trade Network upgrades
  ...
  if(kind === "stability"){
    costGP = 75;
    label = "Bastion Trade Network: Stability Investment";
  } else if(kind === "yield"){
    costGP = 90;
    label = "Bastion Trade Network: Yield Investment";
  } else if(kind === "toggle_high_risk"){
    costGP = 0;
    label = "Bastion Trade Network: Routing Doctrine Change";
  } else {
...
function renderTradeRouteOverlaySvg(clans){
  ...
  // Bastion marker
  lines.push(`<circle class="tradeRouteDot" cx="${TRADE_MAP_OVERLAY.bastion[0]}" cy="${TRADE_MAP_OVERLAY.bastion[1]}" r="9"></circle>`);
...
hallUpgradePill.onclick = () => {
  ...
  <div class="hallBody">
    ...
    ${renderBastionTradeNetworkSection()}
  </div>
`;
...
bindHallFunctionTooltips(hallCard);
  ...
  issueOrder(facId, fnId);
   ...
   // Bastion Trade Network upgrade buttons (consume Bastion Turn via pending order)
...
function renderBastionTradeNetworkSection(){
  ...
  return `
    ...
    <div class="hallName" style="margin-bottom:6px">Bastion Trade Network</div>
...
const CONSORTIUM_NAME = "Bastion Trade Consortium";
...
function bindFacilitySlotTooltips(){
  ...
  if(kind === "consortium"){
    return [
      "Creates a Trade Consortium contract and activates the Bastion Trade Network.",
