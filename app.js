...
const EXPLORER_KEY = "scarlettWorldHub.v1";
const WORLD_CONTENT_URL = withBase("data/world_content.json");
const HUB_ENCOUNTER_KEY = "scarlettWorldHub.encounter.v1";
    // -------------------------------
...
async function loadWorldContentDb(){
  try{
    const res = await fetch(WORLD_CONTENT_URL, { cache: "no-cache" });
    if(!res.ok) throw new Error(`Failed to load world_content.json (${res.status})`);
    WORLD_CONTENT_DB = await res.json();
    return WORLD_CONTENT_DB;
  }catch(err){
    console.warn("World content DB failed to load:", err);
    WORLD_CONTENT_DB = { locations:{}, wildernessHotspotsByMapId:{}, encounterTemplates:{} };
    return WORLD_CONTENT_DB;
  }
}
...
// -------------------------------
const HEROES = [
  { id: "hero_1", title: "Hero 1" },
  { id: "hero_2", title: "Hero 2" },
  { id: "hero_3", title: "Hero 3" },
  { id: "hero_4", title: "Hero 4" }
];
  ...
  view.innerHTML = `
        ...
        <div class="explorer-brand">
          ...
          <div class="explorer-brandText">
            <h1 class="explorer-title">Scarlett Isles World Hub</h1>
            <p class="muted explorer-sub">
              Welcome hub for exploration, encounters, and bastion play across the Scarlett Isles.
            </p>
          </div>
        </div>
      </div>
...
if (saved.travel) state.travel = { ...state.travel, ...saved.travel };
  ...
  function renderMarkers(){
    ...
    renderRegionalMarkersAndHotspots();
  }

  hubPartyCampaignBtn?.addEventListener("click", openPartyOverlay);
  hubOpenBastionBtn?.addEventListener("click", openBastionManager);
  hubExitSubmapBtn?.addEventListener("click", exitSubmap);
  hubPartyClose?.addEventListener("click", () => closePartyOverlay());
  hubPartyOverlayBackdrop?.addEventListener("click", () => closePartyOverlay());
  hubPoiClose?.addEventListener("click", closePoiModal);
  hubPoiBackdrop?.addEventListener("click", closePoiModal);
  ...
  hubAddPartyMember?.addEventListener("click", () => {
    ...
    renderPartyRows(rows);
  });
  hubResetParty?.addEventListener("click", () => {
    renderPartyRows(["Hero 1", "Hero 2", "Hero 3", "Hero 4"]);
  });
  hubPartyLevel?.addEventListener("input", syncPartyOverlayUi);
  hubPartySave?.addEventListener("click", () => {
    const names = Array.from(hubPartyList.querySelectorAll('[data-party-name]')).map(i => i.value || "");
    if (!rebuildPartyTokens(names)) {
      alert("Add at least one named player token.");
      return;
    }
    const lvl = explorerClamp(Number(hubPartyLevel?.value || 1), 1, 20);
    state.campaign.partyLevel = lvl;
    state.campaign.bastionEnabled = lvl >= 5 ? !!hubBastionEnabled?.checked : false;
    state.campaign.bastionName = state.campaign.bastionEnabled ? String(hubBastionName?.value || "").trim() : "";
    state.campaign.started = true;
    if (!state.campaign.bastionEnabled) state.campaign.weeklyPromptDay = 0;
    saveNow();
    rerenderAll();
    updateHubButtons();
    closePartyOverlay(true);
    if (!state.mapPresetId && !state.submap) setNotice("Choose a regional map to begin exploring.");
  });
...
importFile?.addEventListener("change", async () => {
  ...
  if(!looksLikeExplorerSave(obj)){
    alert("That file doesn’t look like a Scarlett Isles World Hub save JSON.");
    importFile.value = "";
    return;
  }
...
if (state.mapDataUrl) {
  ...
  rerenderAll();
    updateFreeMoveUI();
    updateHubButtons();
    if (!state.campaign?.started) openPartyOverlay();
  // Load events in the background
loadEventDb();
    Promise.all([loadMarkerDb(), loadWorldContentDb()]).then(() => renderMarkers());
...
/* NEW: every 7 days, optionally prompt for a bastion turn */
if (state.campaign?.bastionEnabled) {
...
if (state.freeMove || state.submap) {
  setNotice(state.submap ? "Local submap movement: travel miles are paused here." : "Free Move: repositioned without spending miles.");
  drag = null;
  saveNow();
  rerenderAll();
  return;
